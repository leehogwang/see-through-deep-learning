...
export function setDynamicBlocks(blocks) {
...
export function getCategoryCatalog() {
    ...
    const dynamicCategories = [...new Set(getBlockCatalog()
...
export function inferParamMeta(blockType, key, value) {
    ...
    if (typeof value === 'string') {
        if (key === 'shape')
            return { kind: 'shape', placeholder: value };
        if (value.includes(','))
            return { kind: 'tuple', placeholder: value };
        if (value === 'true' || value === 'false')
            return { kind: 'bool' };
        if (/^-?\d+$/.test(value.trim()))
            return { kind: 'int', step: 1 };
...
function iconForCategory(category) {
    ...
    if (category.includes('container'))
        return '🧱';
    if (category.includes('padding'))
        return '🪟';
    if (category.includes('upsample'))
        return '🪄';
    if (category.includes('distance'))
        return '📐';

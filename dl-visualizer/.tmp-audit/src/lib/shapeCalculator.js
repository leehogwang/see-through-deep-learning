function canonicalBlockType(blockType) {
...
function convOut(size, kernel, stride, padding, dilation = 1) {
    return Math.floor((size + 2 * padding - dilation * (kernel - 1) - 1) / stride) + 1;
}
function normalizeInputShape(inputShape, inputShapes) {
    return inputShape ?? inputShapes.find((shape) => Array.isArray(shape)) ?? null;
}
function parseAxisValues(value, dims, fallback) {
    ...
    if (typeof value === 'string') {
        ...
        if (values.length === 1)
            return Array(dims).fill(values[0]);
...
function sumConcatShapes(shapes, dim) {
    if (shapes.length === 0)
        return null;
    const merged = [...shapes[0]];
    for (const shape of shapes.slice(1)) {
        if (shape.length !== merged.length)
            return null;
...
export function calcOutputShape(blockType, params, inputShape, inputShapes = []) {
    blockType = canonicalBlockType(blockType);
    const err = (msg) => ({ shape: null, error: true, str: msg });
    const ok = (shape) => ({ shape, error: false, str: shapeStr(shape) });
    if (blockType === 'Input') {
        return ok(parseShape(String(params.shape || 'B,3,224,224')));
    }
    const resolvedInputShape = normalizeInputShape(inputShape, inputShapes);
    if (blockType === 'Output')
        return resolvedInputShape ? ok([...resolvedInputShape]) : err('no input');
    if (!resolvedInputShape)
        return err('no input');
    switch (blockType) {
        case 'Conv1D': {
            if (resolvedInputShape.length !== 3)
                return err('needs (B,C,L)');
            const inChannels = parseNumber(params.in_ch, resolvedInputShape[1]);
            if (resolvedInputShape[1] !== -1 && inChannels !== resolvedInputShape[1])
                return err(`expected C=${inChannels}, got ${resolvedInputShape[1]}`);
        ...
        case 'DilatedConv2D': {
            if (resolvedInputShape.length !== 4)
                return err('needs (B,C,H,W)');
            const inChannels = parseNumber(params.in_ch, resolvedInputShape[1]);
            if (resolvedInputShape[1] !== -1 && inChannels !== resolvedInputShape[1])
                return err(`expected C=${inChannels}, got ${resolvedInputShape[1]}`);
        ...
        case 'Conv3D': {
            if (resolvedInputShape.length !== 5)
                return err('needs (B,C,D,H,W)');
            const inChannels = parseNumber(params.in_ch, resolvedInputShape[1]);
            if (resolvedInputShape[1] !== -1 && inChannels !== resolvedInputShape[1])
                return err(`expected C=${inChannels}, got ${resolvedInputShape[1]}`);
        ...
        case 'LinearAttention': {
            ...
            if (numHeads > 0 && embedDim > 0 && embedDim % numHeads !== 0)
                return err(`embed_dim ${embedDim} not divisible by num_heads ${numHeads}`);
            return ok([...resolvedInputShape]);
        }
        case 'LSTM':
        case 'GRU':

import { getBlockCatalog, type BlockDef, type ParamValue } from '../src/data/blocks.js'
import { calcOutputShape, type Shape } from '../src/lib/shapeCalculator.js'
...
function chooseInputShapes(block: BlockDef, candidate: Candidate): Shape[] {
  const maxInputs = block.maxInputs ?? 1
  return Array.from({ length: maxInputs }, () => [...candidate.shape])
}

function autoTuneParams(block: BlockDef, inputShape: Shape): Record<string, ParamValue> {
  const params: Record<string, ParamValue> = { ...block.defaultParams }
  ...
  if (type === 'conv1d' || type === 'conv2d' || type === 'conv3d' || type === 'depthwiseconv2d' || type === 'dilatedconv2d' || type === 'transposedconv2d') {
    if (channelDim > 0) params.in_ch = channelDim
  }
  ...
  if (type === 'groupnorm') {
    ...
    if (channelDim > 0 && groups > channelDim) params.num_groups = channelDim
  }
  ...
  if (type === 'multiheadattention' || type === 'selfattention' || type === 'crossattention' || type === 'flashattention' || type === 'gqa' || type === 'mqa' || type === 'linearattention') {
    ...
    while (heads > 1 && featureDim > 0 && featureDim % heads !== 0) heads -= 1
...
function auditBlock(block: BlockDef): AuditResult {
  if (block.type === 'Input') {
